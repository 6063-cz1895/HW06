...
let currentLine; // Define a variable for the current line of text
...
// Set function for the text part of the poem (see readme)
function wave(startX, startY) {//(startX, startY) is the starting point of the text
  let x = startX; // set x to the starting point
  for (let i = 0; i < currentLine.length; i++) {//because this effect should be happenning to each of the letters, so when i is smaller than the number of letters, it should ++
    ...
    x += textWidth(currentLine[i]); // move right for each letter
    text(currentLine[i], x, y); // draw text
  }
}
...
function draw() {
  ...
  for (let yPosition = height * 1 / 5; yPosition < height; yPosition += waveSpacing, xPosition += 280) {//draw a for loop to repeat the poem untill the end of the screen, also move right a bit for every repeat
    for (let i = 0; i < txt.length; i++) {
      currentLine = txt[i]; // Set the current line to the text from the array
      wave(xPosition, yPosition); // draw our wavey text
